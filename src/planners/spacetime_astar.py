"""
多维时空 A* 路径规划算法 (3.25 核心算法 / 任务书 §八 推荐技术路线)
...
任务书 §五.3：
"静态障碍避障：搜索过程中禁止进入障碍网格。生成绕障路径。计算路径长度和避障代价。"

本算法特点：
- 四维状态空间 (ix, iy, iz, it)
- 6/26 邻域空间移动 + 等待动作（每次推进 1 个时间步）
- 启发函数：曼哈顿距离 × dt（可采纳）
...
接口：
- plan(start, goal, start_time, ...): 异步执行 A* 搜索，返回 PlannedRoute
"""
