...
def main():
    parser = argparse.ArgumentParser(description="低空无人机路径规划调度")
    parser.add_argument("--data-dir", default="./data", help="输入数据目录")
    parser.add_argument("--output-dir", default="./output", help="输出目录")
    parser.add_argument("--mode", choices=["spacetime", "optimize", "both", "batch"], default="both",
        ...
        for i, task in enumerate(tasks):
            ...
            if len(task.waypoints) > 2:
                ...
                if success:
                    ...
                    print(f"    [{task.task_id}] 成功: {route.total_distance:.1f}m, {route.total_time:.1f}s, {len(traj)}点")
                else:
                ...
                if route.status == "planned":
                    print(f"    [{task.task_id}] 成功: {route.total_distance:.1f}m, {route.total_time:.1f}s")
                else:
    ...
    if args.mode in ("optimize", "both"):
        ...
        for i, task in enumerate(tasks):
            ...
            if route.status == "planned":
                print(f"    [{task.task_id}] 优化完成: {route.total_distance:.1f}m, {route.total_time:.1f}s")
                print(f"      风险: max={route.max_risk:.4f}, avg={route.avg_risk:.4f}, 最近障碍={route.min_obstacle_distance:.1f}m")
                # 生成航点
    ...
    if args.mode == "batch":
        print("\n  --- 大规模并行规划 ---")
        planner = ParallelPlanner(config, obstacle_set, max_workers=4)
        result = planner.plan_batch(tasks, performances, request_id="batch-demo")
        all_routes.extend(result.routes)
        print(f"    批量规划: {result.success_count}成功 / {result.failed_count}失败, 耗时 {result.total_planning_time_ms:.1f}ms")
    ...
    for route in all_routes:
        if route.status == "planned":
            write_route(route, os.path.join(output_dir, "routes"))
    ...
    # 只展示优化后的航线（如果有）
    opt_routes = [r for r in all_routes if r.algorithm and "Optimizer" in r.algorithm and r.status == "planned"]
    display_routes = opt_routes if opt_routes else [r for r in all_routes if r.status == "planned"]

    if display_routes:
        # Matplotlib 3D 图
        img_path = os.path.join(output_dir, "routes_3d.png")
        visualizer.plot_matplotlib_3d(display_routes, img_path, title="低空无人机路径规划 3D 可视化")
        print(f"  3D 静态图: {img_path}")

        # Plotly 交互式 HTML
        html_path = os.path.join(output_dir, "routes_interactive.html")
        visualizer.generate_plotly_html(display_routes, html_path, title="低空无人机路径规划交互式可视化")
        print(f"  交互式 HTML: {html_path}")

        # 风险热力图（第一条航线）
        heatmap_path = os.path.join(output_dir, "risk_heatmap.png")
        visualizer.plot_risk_heatmap(display_routes[0], heatmap_path)
        print(f"  风险热力图: {heatmap_path}")
    else:
        print("  无成功规划的航线，跳过可视化")
    ...
    print("\n" + "=" * 60)
    print(f"  规划完成! 成功 {result.success_count}/{len(all_routes)} 条航线")
