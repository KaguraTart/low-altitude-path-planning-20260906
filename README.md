...
|--------|------|--------|------------|-----------------|------------|---------------|
| 10     | 10/10   | 100.0% | 0.15 s   | 0.08 s   | 66.9/s   | 117.7/s |
| 100    | 90/100  | 90.0%  | 4.82 s   | 4.83 s   | 20.8/s   | 20.7/s  |
| 1,000  | 925/1000 | 92.5% | 39.26 s  | 39.14 s  | 25.5/s   | 25.6/s  |
| 10,000 | 9,143/10,000 | 91.43% | 462.49 s (7.7 min) | 460.71 s (7.7 min) | 21.6/s | 21.7/s |

**累计 11,110 条任务**，整体成功率 **91.52%**，两种模式结果一致（DISPATCH 与 FULL 成功率相同，仅耗时微差）。

> 失败主要来自：A* 起点被障碍物占用（DISPATCH 预校验救回约 1%）、多航点中间航点被逼入死路、A* 达到 50,000 次迭代上限仍未找到路径。
...
**结论**：**90% 的任务在 50ms 内完成**（P50 ≈ max_speed×邻域代价），但 **P95/P99 达到数秒**（说明 5-10% 的"硬骨头"任务会啃 A\* 迭代上限）。墙钟时间主要被尾延迟拖慢。
...
|--------|----------|---------------|---------|--------------|------------------|
| 10     | 0.15     | 0.08          | 1.88×  | 100.0%      | 100.0%          |
| 100    | 4.82     | 4.83          | 1.00×  | 90.0%       | 90.0%           |
...
> 实测提速很小（1% 量级），原因是 A\* 主循环占了 90%+ 时间，risk/heading 仅占 1-3%。
> 主要价值：**节省 I/O 和报告生成开销** + **救回更多失败任务**（起点预校验）。
